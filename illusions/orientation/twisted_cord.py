"""Twisted Cord Illusion (orientation, VI-Probe case 25). Straight vertical poles filled with diagonal black-and-white stripes appear twisted or spiral-shaped like twisted cords."""
...
class TwistedCordIllusion(IllusionTemplate):
    """
    Twisted Cord Illusion Generator

    Generates the twisted cord illusion using straight vertical poles filled with
    diagonal stripes that create the perception of twisting or spiraling.

    The strength parameter = stripe angle (degrees):
    - strength = 0°: Vertical stripes (no illusion)
    - strength = 30°: Mild diagonal stripes
    - strength = 45°: Standard diagonal stripes (strong illusion)
    - strength = 60°: Steep diagonal stripes
    """
    ...
    def __init__(self,
        ...
        self.BASE_STRIPE_WIDTH = STRIPE_WIDTH  # Base stripe width for Control/Original modes
        ...
        super().__init__(
            illusion_name="twisted_cord_illusion",
            width=WIDTH,
            height=HEIGHT,
            strength_levels=[0.5, 0.8, 1.0, 1.2, 1.5, 2.0],  # Stripe width multipliers for Control/Original
            background_color=(0.9, 0.9, 0.9)  # Light gray background
        )
    ...
    def generate_stripe_pattern(self,
        ...
        Args:
            ...
            colors: List of colors to alternate (typically 2 colors for black/white)
    ...
    def define_elements(self, strength: float, is_original: bool) -> Dict[str, Any]:
        ...
        base_stripe_pattern = self.generate_stripe_pattern(
            ...
            angle=stripe_angle,
            colors=[self.BLACK_COLOR, self.WHITE_COLOR]
        )
    ...
    def generate_illusion(self, image: np.ndarray, elements: Dict[str, Any]) -> np.ndarray:
        ...
        Draws each cord as a vertical rectangle filled with diagonal stripes,
        ...
        # Draw ground line (horizontal black rectangle at the bottom of cords)
        ...
        if ground_y + ground_height <= self.height:
            image[ground_y:ground_y + ground_height, :] = ground_color
    ...
    def _draw_twisted_cord(self, image: np.ndarray,
        ...
        """
        Draw a single twisted cord (pole with diagonal stripes).
        ...
        # Use pre-generated stripe pattern (or white for control mode)
        if control_mode:
            # Control mode: solid white rectangle
            stripe_pattern = np.ones((cord_height, cord_width, 3), dtype=np.float32)
        else:
    ...
    def apply_perturbation(self, elements: Dict[str, Any]) -> Dict[str, Any]:
        """
        Apply perturbation by reversing the stripe direction.

        The perturbed version uses opposite diagonal direction (mirror effect),
        which also creates the twist illusion but in the opposite visual direction.

        Args:
            elements: Elements with original stripe direction

        Returns:
            Modified elements (already set in define_elements for perturbed mode)
        """
        # Perturbation is already applied in define_elements()
        # (opposite stripe angle for perturbed mode)
        return elements
