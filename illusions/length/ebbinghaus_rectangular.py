"""Ebbinghaus Illusion Rectangular (length, VI-Probe case 7). Ebbinghaus (Titchener Circles) illusion with surrounding circles arranged in a rectangular layout instead of a circular one."""
...
class EbbinghausIllusionRectangular(IllusionTemplate):
    """
    Ebbinghaus Illusion Generator - Rectangular Layout

    Generates the Ebbinghaus (Titchener Circles) illusion with rectangular surrounding circle layout:
    - Two central circles that appear different sizes due to context
    - Left circle surrounded by 12 small circles (3 per side in rectangular pattern)
    - Right circle surrounded by 12 large circles (3 per side in rectangular pattern)
    ...
    def __init__(self, DEFAULT_CENTER_RADIUS=25):
        ...
        self.LEFT_SMALL_CIRCLE_COUNT = 12  # Rectangular layout: 3 per side
        ...
        self.RIGHT_LARGE_CIRCLE_COUNT = 12  # Rectangular layout: 3 per side

        # Distance multiplier for surrounding circles
        self.SURROUNDING_DISTANCE_FACTOR = 1.3  # Multiplier for spacing (rectangular layout needs more space)

        super().__init__(
            illusion_name="ebbinghaus_rectangular",
            width=512,
            height=256,
    ...
    def _calculate_surrounding_positions_rectangular(self, center_x: int, center_y: int,
                                                     center_radius: int, surrounding_radius: int,
                                                     num_circles: int) -> List[Tuple[int, int]]:
        """
        Calculate positions for surrounding circles arranged in a rectangular pattern.
        ...
        Args:
            ...
            num_circles: Total number of surrounding circles (must be multiple of 4)

        Returns:
            List of (x, y) positions for surrounding circles in rectangular layout
        """
        positions = []
        distance = self.SURROUNDING_DISTANCE_FACTOR * (center_radius + surrounding_radius)

        # Number of circles per side
        circles_per_side = num_circles // 4

        # Top edge (left to right)
        for i in range(circles_per_side):
            if circles_per_side == 1:
                x = center_x
            else:
                x = center_x - distance + (2 * distance * i / (circles_per_side - 1))
            y = center_y - distance
        ...
        for i in range(circles_per_side):
            ...
            if circles_per_side == 1:
                ...
                y = center_y - distance + (2 * distance * i / (circles_per_side - 1))
            positions.append((int(x), int(y)))
    ...
    def define_elements(self, strength: float, is_original: bool) -> Dict[str, Any]:
        """
        Define Ebbinghaus illusion elements with rectangular surrounding circle layout.
        ...
        # Calculate surrounding circle positions (rectangular layout)
        left_surrounding_positions = self._calculate_surrounding_positions_rectangular(
        ...
        right_surrounding_positions = self._calculate_surrounding_positions_rectangular(
        ...
        elements = {
            ...
            # Left surrounding small circles (rectangular layout)
            ...
            # Right surrounding large circles (rectangular layout)
    ...
    def apply_perturbation(self, elements: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Recalculate left surrounding circle positions (rectangular layout)
